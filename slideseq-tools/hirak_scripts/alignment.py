...
for library_index in range(n_libraries):
    for run_info in run_info_list:
        for lane in run_info.lanes:
            ...
            print('Running for ', sample.name, sample.lane)
            ...
            cmd = ' '.join(cmd)
            drop_seq_cmd_1[(library_index,run_info.flowcell,lane)] = cmd
            ...
            cmd = [str(arg) for arg in cmd]
            cmd = ' '.join(cmd)
            drop_seq_cmd_2[(library_index,run_info.flowcell,lane)] = cmd
            ...
            cmd = [str(arg) for arg in cmd]
            cmd = ' '.join(cmd)
            drop_seq_cmd_3[(library_index,run_info.flowcell,lane)] = cmd
            ...
            if sample.start_sequence != constants.NO_START_SEQUENCE:
                ...
                cmd = [str(arg) for arg in cmd]
                cmd = ' '.join(cmd)
                drop_seq_cmd_4[(library_index,run_info.flowcell,lane)] = cmd
            ...
            cmd = [str(arg) for arg in cmd]
            cmd = ' '.join(cmd)
            drop_seq_cmd_5[(library_index,run_info.flowcell,lane)] = cmd
            ...
            cmd = [str(arg) for arg in cmd]
            cmd = ' '.join(cmd)
            picard_cmd_1[(library_index,run_info.flowcell,lane)] = cmd
            
            
            # ----------------------------------
            
            cmd = [
                "/home/hsarkar/code/slideseq-tools/soft/STAR-2.7.10a/bin/Linux_x86_64_static/STAR",
                "--genomeDir",
            ...
            print('Calculating stats -- awefully slow!!')
            # write_alignment_stats(sample.aligned_bam, sample.alignment_pickle)
    
            cmd = [str(arg) for arg in cmd]
            cmd = ' '.join(cmd)
            star_cmd[(library_index,run_info.flowcell,lane)] = cmd
