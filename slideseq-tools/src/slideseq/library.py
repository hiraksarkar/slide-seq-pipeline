...
class Library:
    ...
    @property
    def raw_barcode(self) -> List[Path]:
        """Final aligned+unmapped output BAMs"""
        return self.per_sample(suffix=".barcode.file")
...
class Sample(Library):
    ...
    @property
    def recovered_ubam(self) -> Path:
        """Raw unmapped BAM. Keep this one for posterity"""
        return (self.lane_dir / self.base).with_suffix(".unmapped.recovered.bam")
    
    @property
    def raw_barcode(self) -> Path:
        """Raw unmapped BAM. Keep this one for posterity"""
        return (self.lane_dir / self.base).with_suffix(".barcode.file")
